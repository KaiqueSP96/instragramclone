import { Avatar, AvatarGroup, Flex, Text, VStack, Button } from "@chakra-ui/react"

const ProfileHeader = () => {
  return (
  <Flex gap={{base:4, sm:10}} py={10} direction={{base:'column', sm:'row'}}>
    <AvatarGroup size={{base: 'xl', md:'2xl'}} justifySelf={'center'} alignSelf={'flex-start'} mx={'auto'}>
        <Avatar name="_romeoproggramer" src='/public/profilepic.png'/>
    </AvatarGroup>

    <VStack alignItems={'start'} gap={5} mx={'auto'} flex={1}>
        <Flex gap={6} direction={{base: 'column', sm:'row'}}
              justifyContent={{base: 'center', sm: 'flex-start'}}
              alignItems={'center'} w={'full'}
        >

          <Text fontSize={{base: 'md', sm: 'lg'}}>
          _romeoproggramer
          </Text>

          <Flex gap={4} alignItems={'center'} justifyContent={'center'}>
            <Button bg={'white'} color={'black'} _hover={{bg: 'whiteAlpha.800'}} size={{base: 'xs', md:'sm'}}>
              Edit profile
            </Button>
          </Flex>
        </Flex>

    <Flex alignItems={'center'} gap={{base:2, sm:4}}>
      <Text fontSize={{base: 'xs', md:'sm'}}>
        <Text as={'span'} fontWeight={'bold'} mr={1}>5</Text>
        Posts
      </Text>
      <Text fontSize={{base: 'xs', md:'sm'}}>
        <Text as={'span'} fontWeight={'bold'} mr={1}>98</Text>
        Followers
      </Text>
      <Text fontSize={{base: 'xs', md:'sm'}}>
        <Text as={'span'} fontWeight={'bold'} mr={1}>200</Text>
        Following
      </Text>
    </Flex>

    <Flex alignItems={'center'} gap={4}>
      <Text fontSize={'sm'} fontWeight={'bold'}>As a junior programmer.</Text>
    </Flex>

    <Flex alignItems={'center'} gap={4}>
      <Text fontSize={'sm'}>Videos and tutotials from Youtube.</Text>
    </Flex>

    </VStack>
  </Flex>
  )
}

export default ProfileHeader